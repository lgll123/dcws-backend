...
public class ExceptionUtil {
  ...
  private static final Map<String, ExceptionEnum> EXCEPTION_ENUM_MAP = Arrays.stream(
          ExceptionEnum.values())
      .collect(Collectors.toMap(ExceptionEnum::getExceptionCode, Function.identity(), (x, y) -> y));

  public static KstryException buildException(Throwable exception,
      @Nonnull ExceptionEnum exceptionEnum, String desc) {
  ...
  public static String getExcMessage(String code, String desc) {
    ...
    return GlobalUtil.format("[{}] {}",
        StringUtils.isBlank(code) ? ExceptionEnum.SYSTEM_ERROR.getExceptionCode() : code,
        StringUtils.isBlank(desc) ? defErrorDesc : desc);
  }

  public static Optional<String> tryGetCode(Throwable throwable) {
    if (throwable instanceof KstryException) {
      return Optional.ofNullable(
          GlobalUtil.transferNotEmpty(throwable, KstryException.class).getErrorCode());
    }
