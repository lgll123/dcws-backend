...
import com.formssi.workflow.mapper.ActTaskMapper;
...
import com.formssi.workflow.utils.ModelUtils;
...
import static com.formssi.workflow.common.constant.FlowConstant.*;
...
public class ActTaskServiceImpl implements IActTaskService {
    ...
    public Map<String, Object> startWorkFlow(StartProcessBo startProcessBo) {
        ...
        WfDefinitionConfigVo wfDefinitionConfigVo = wfDefinitionConfigService.getByTableNameLastVersion(startProcessBo.getRouter());
    ...
    @Override
    public boolean checkRollback(RollbackProcessBo rollbackProcessBo) {
        String userId = String.valueOf(LoginHelper.getUserId());
        ProcessInstance processInstance = QueryUtils.businessKeyQuery(rollbackProcessBo.getBusinessKey()).singleResult();
        if (processInstance == null) {
            return false;
        }
        String processInstanceId = processInstance.getProcessInstanceId();
        //校验单据
        if (!BusinessStatusEnum.WAITING.getStatus().equals(processInstance.getBusinessStatus())) {
            return false;
        }
        ...
        //获取回滚任务
        HistoricTaskInstance prevTask = getPrevTask(processInstanceId, task);
        //上个任务审核是否自己
        return prevTask != null && userId.equals(prevTask.getAssignee());
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean rollbackTask(RollbackProcessBo rollbackProcessBo) {
        String userId = String.valueOf(LoginHelper.getUserId());
        try {
            ProcessInstance processInstance = QueryUtils.businessKeyQuery(rollbackProcessBo.getBusinessKey()).singleResult();
            String processInstanceId = processInstance.getProcessInstanceId();
            ...
            if (!taskList.isEmpty()) {
                ...
                throw new ServiceException(MESSAGE_NOT_ROLLBACK);
            }
            //获取回滚任务
            HistoricTaskInstance prevTask = getPrevTask(processInstanceId, task);
            //上个任务审核是否自己
            if (prevTask == null || !userId.equals(prevTask.getAssignee())) {
                throw new ServiceException(MESSAGE_NOT_ROLLBACK);
            }
            taskService.addComment(task.getId(), processInstanceId, TaskStatusEnum.ROLLBACK.getStatus(), "撤回");
            //判断是否有多个任务
            if (taskList.size() > 1) {
                //当前多个任务撤回到单个节点
                runtimeService.createChangeActivityStateBuilder().processInstanceId(processInstanceId).moveActivityIdsToSingleActivityId(taskList.stream().map(Task::getTaskDefinitionKey).distinct().collect(Collectors.toList()), prevTask.getTaskDefinitionKey()).changeState();
                ...
                //当前单个节点撤回单个节点
                runtimeService.createChangeActivityStateBuilder().processInstanceId(processInstanceId).moveActivityIdTo(task.getTaskDefinitionKey(), prevTask.getTaskDefinitionKey()).changeState();
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw new ServiceException(e.getMessage());
    ...
    private HistoricTaskInstance getPrevTask(String processInstanceId, Task task) {
        HistoricTaskInstance prevTask = null;
        ...
        if (historicTasks != null && historicTasks.size() > 2) {
            for (int i = 0; i < historicTasks.size(); i++) {
                HistoricTaskInstance historicTask = historicTasks.get(i);
                if (task.getId().equals(historicTask.getId()) && historicTask.getEndTime() == null) { //未审批，可撤回
                    prevTask = historicTasks.get(i - 1);
                }
            }
        }
        return prevTask;
    ...
    @Override
    public List<WfFormPermissionVo> getFormPermission(FormStatusBo formStatusBo) {
        if (StringUtils.isNotBlank(formStatusBo.getTaskId())) {
            return WorkflowUtils.getFormPermissionByTaskId(formStatusBo.getTaskId());
        } else if (StringUtils.isNotBlank(formStatusBo.getRouter())) {
            WfDefinitionConfigVo wfDefinitionConfigVo = wfDefinitionConfigService.getByTableNameLastVersion(formStatusBo.getRouter());
            if (wfDefinitionConfigVo == null) {
                throw new ServiceException("请到流程定义绑定业务表名与流程KEY！");
            }
            return WorkflowUtils.getFormPermissionByProcessKey(wfDefinitionConfigVo.getProcessKey());
        }
